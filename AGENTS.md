...
7. **密钥安全 + 正文可观测**：API key 鉴权只依赖 **sha256 哈希**；如配置了 at-rest 加密密钥，可额外保存 `secret_enc`（AES-GCM 密文）供管理后台查看/轮转。Key Store、遥测、payload 表与常规日志绝不保存明文 key；唯一例外是首次 bootstrap 可把 root key 打印一次，并原子写入配置的 `0600` recovery file，之后不会再次显示。**默认按 Session 增量保存会话转录；完整 request/response 正文仅在运行时设置 `capture_payloads` 开启时记录到独立的 `request_payloads` 表**。两种模式互斥，可在管理界面「系统设置」切换，并按 `payload_retention_days` 自动清理。`DecisionRecord` 仍保持脱敏（不含正文）作为纵深防御。
