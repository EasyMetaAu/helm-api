...
## 2026-06-20 · 记忆形成低延迟：防抖唤醒 memory worker（docs/08 Phase 2；原则 3）

- **背景/线上实证**：用户在 la.atmy.work 说「我喜欢 42」后期望记忆很快形成，实测 ~55s。box DB 铁证：observer job `0442719a` **10:52:59 入队 → 10:53:54 完成 = 55.2s**；同期每分钟 `:54` 那批 job delta 仅 ~5ms（worker tick 自产自销的 idle-flush，非请求触发）。根因：memory worker 用 `setInterval` 每 **60s** 轮询 `claimPendingJobs`，请求路径在 inject 阶段**立即入队** observer job（`chat.ts` → `enqueueObserverWriteback` 直写 DB），但要**干等下一个 tick** 才被捞起 → 最坏 60s 纯排队。这是架构刻意把 worker 放请求路径外（zero added latency）的代价。
- **关键取舍（用户在 4 选项里选「防抖唤醒」而非「立即」）**：延迟与省钱是**同一个旋钮**——`enqueueJob`（`INSERT OR IGNORE` + open-job 唯一索引）对「同线程未完成 job」幂等去重，故一个 job 在 `pending` 期间后续每个回合都 dedupe 进它，drain 时**一次** observer run 覆盖全部累积消息 = **一次** LLM 调用（事实抽取仅在有新用户消息时发；压缩仅跨阈值时发）。「job 在 pending 待多久」就是合并窗口。立即 drain 会让合并失效（≈每回合一次记忆 LLM 调用）；**防抖 drain** 既降延迟又保住合并：observe 落库后等一个静默窗口（`coalesceMs` 默认 8s）再 drain，窗口内的新回合仍 dedupe 进同一 job 合并成一次调用。
- **实现**：① `scheduler.ts` 把 tick 拆成 `drainJobs`（claim+processJob，**无** onTick）与 `tick`（onTick + drain）；`MemoryWorkerHandle` 加 `wake()`=trailing-edge 防抖（每次 `clearTimeout`+重设 `coalesceMs` timer，`unref`，`stop()` 清掉）；`draining`+`rerun` 防重入（drain 期间触发→结束后补跑一轮；claimPendingJobs 本身原子 UPDATE…RETURNING 故仅避免空转非正确性）；wake-drain **不**跑 onTick（housekeeping 仍按 60s）。② `write-queue.ts` 加可选 `onTaskDrain?`，在 enqueueTask 的 finally fail-open 调用——两条路由（chat/messages-pipeline）的 observe 都走同一 `taskChain`，故**一个接线点全覆盖** OpenAI/Anthropic/Responses 三面。③ `server.ts` 用迟绑定 `wakeMemoryWorker`（createWriteQueue 早于 startMemoryWorker）：`onTaskDrain: () => wakeMemoryWorker()`，worker 建好后 `wakeMemoryWorker = () => memoryWorker?.wake()`；`coalesceMs` 经 `HELM_MEMORY_WORKER_COALESCE_MS`（默认 8000，钳到 `<intervalMs`）。**唤醒必在消息落库后**（observer 读 listMessages，job 行 inject 阶段已先建好）——onTaskDrain 在 observe task 完成后触发，时序天然正确。
- **坑/限制/TODO**：① 60s `intervalMs` 定时器**保留为 backstop/maxWait**：持续活动下防抖不断重置，但定时器每 60s 仍强制 drain → 最坏延迟 ≤60s（不劣于今天）。② **未拆分** eager/compaction 路径（用户选防抖方案，非「事实即时+压缩批量」）；observer/抽取逻辑本身未动——延迟纯在调度层。③ **无 schema 改/无迁移**，box 配置仍有效、fail-close gate N/A；box 可选设 `HELM_MEMORY_WORKER_COALESCE_MS` 覆盖。④ **未部署**；box 的 `helm.db` 已 **9.8GB**（telemetry/request_payloads 留存），另案排查，部署不触碰。
- **验证**：TDD 红→绿，scheduler 7 新测（防抖/突发合并/wake 不跑 onTick/fail-open/stop 取消/interval backstop 仍 drain）+ write-queue 2 新测（onTaskDrain 每 task 触发/抛错不污染链）；memory+gateway runtime **260 测**绿、typecheck（4 项目）/lint(0/0)/build 清。分支 `feat/memory-worker-debounced-wake`。
