...
- **验证**：TDD 红→绿，dimensions.test.ts 新增 4 用例（大系统提示不抬 rawScore<0.30、msg_length 取末条、末条代码块仍触发、ambient turn/tool 仍全请求）+ golden-routing 一条 Mimi 多消息 e2e（chat/非 complex/非 premium-coding）。**真实正文** `/tmp/req_5ee4.json` 经修复 `scoreRequest`→ task=chat、**complexity `complex`→`simple`**（仅剩 ambient 命中）→ economy lane。classifier 全套+samples **274** 绿（config 未改）、typecheck(4)/lint(479) 清。分支 `fix-classifier-tier-scope-last-user`，未部署。
