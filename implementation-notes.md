...
- 本次同时在 OAuth pool 的流内首个输出等待点加入明确过载的同账号短退避（最多 1 秒、3 秒两次），复用请求取消信号；生产请求总预算为 60 秒，而过载样本大量耗时约 31 秒；该退避只作用于无 previous_response_id 的非严格亲和流，并在随后转入既有 sibling/model fallback 前耗尽；仍需后续把 HTTP、账号池与模型层的次数/时间统一成显式预算并记录各层原因，不能叠加重试。当前失败响应正文不完整，不能把本地复现直接当作每条生产错误的确定根因。
