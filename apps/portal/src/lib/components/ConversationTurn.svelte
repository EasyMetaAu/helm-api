<script lang="ts">
  ...
  const firstToolPart = $derived(
    turn.parts.findIndex(
      (p) =>
        p.kind === "tool_exchange" ||
        p.kind === "tool_result" ||
        p.kind === "tool_call",
  ...
  const ROLE: Record<ConversationTurn["role"], RoleStyle> = {
    user: {
      dot: "bg-slate-500",
      spine: "border-slate-200",
      name: "text-ink-strong",
    },
    assistant: {
      dot: "bg-brand",
      spine: "border-indigo-200",
      name: "text-brand",
    },
    system: {
      dot: "bg-amber-500",
      spine: "border-amber-200",
      name: "text-amber-700",
    },
  ...
  const preview = $derived.by(() => {
    ...
    if (ex && ex.kind === "tool_exchange")
      return `${ex.name || $t("tool call")}(${formatToolArgs(ex.name, ex.args)})`;
    ...
    if (turn.parts.some((p) => p.kind === "tool_result"))
  ...
  const sizeHint = $derived.by(() => {
    ...
    for (const p of turn.parts) {
      if (p.kind === "text" || p.kind === "reasoning") chars += p.text.length;
        ...
        chars += approxLen(p.args) + approxLen(p.output);
    }
    if (chars < 1000) return "";
    return chars < 10000
      ? `${Math.round(chars / 100) / 10}k`
  ...
  function isErrorOutput(v: unknown): boolean {
    const s =
      typeof v === "string" ? v : approxLen(v) > 0 ? JSON.stringify(v) : "";
    return (
      /"?(error|is_error|isError)"?\s*[:=]\s*(true|"[^"]|')/i.test(s) ||
      /^error\b/i.test(s.trim())
    );
  }
  function exchangeStatus(p: Extract<TurnPart, { kind: "tool_exchange" }>): {
    ...
    if (!p.hasResult)
      return { glyph: "⋯", cls: "text-ink-faint", label: $t("no result") };
    if (isErrorOutput(p.output))
      return { glyph: "✗", cls: "text-red-600", label: $t("error") };
  ...
  {#if !toolOnly}
    <div class="flex items-center gap-2">
      ...
      >
        ...
        {#if !grouped}
          <span class={`shrink-0 text-xs font-semibold ${style.name}`}
        ...
        <span
          class="flex shrink-0 items-center gap-1.5 text-[11px] text-ink-faint"
        >
          {#if counts.reasoning}<span title={$t("Reasoning")}>🧠</span>{/if}
          {#if counts.tools}<span title={$t("tool call")}
              >🔧{counts.tools > 1 ? `×${counts.tools}` : ""}</span
            >{/if}
          {#if counts.images}<span title={$t("Image")}
              >🖼{counts.images > 1 ? `×${counts.images}` : ""}</span
            >{/if}
          {#if counts.errors}<span class="text-red-500" title={$t("error")}
              >⚠</span
            >{/if}
  ...
  {#if open}
    <div class="pb-2 pl-4 text-sm text-ink-body">
      {#each turn.parts as part, i (partKey(i))}
        {#if part.kind === "text"}
          <p class="my-1 whitespace-pre-wrap break-words leading-relaxed">
            {part.text}
          </p>
        {:else if part.kind === "reasoning"}
          <details
            ...
            open={showReasoning}
          >
            <summary class="cursor-pointer text-xs font-medium text-violet-600"
              >🧠 {$t("Reasoning")}</summary
            >
            <p
              class="mt-1 whitespace-pre-wrap break-words border-l-2 border-violet-200 pl-3 text-xs text-ink-muted"
            >
              {part.text}
            </p>
          </details>
        ...
        {:else if part.kind === "tool_exchange"}
          ...
          {@const detail = formatToolArgs(
            part.name,
            part.args,
            EXPANDED_ARG_CHARS,
          )}
          {@const peek = part.hasResult
            ? toolOutputPeek(part.output, PEEK_LINES)
            : { lines: [], moreLines: 0 }}
          ...
          <div data-testid="conversation-tool" class="my-1.5">
              ...
              >
                ...
                <span class="min-w-0 flex-1 break-words">
                  <span class="font-semibold text-ink-strong"
                    >{part.name || $t("tool call")}</span
                  ><span class="text-ink-muted">({detail})</span>
                </span>
                <span class={`shrink-0 font-medium ${st.cls}`}
            ...
            {#if !full && peek.lines.length}
              <div
                class="mt-0.5 border-l-2 border-border pl-3 font-mono text-[11px] leading-snug text-ink-muted"
              >
            ...
            {#if !full}
              ...
              >
                {peek.moreLines > 0
                  ? `… +${peek.moreLines} ${$t("lines")} (${$t("click to expand")})`
                  : `⋯ ${$t("view details")}`}
              </button>
            {:else}
              <div class="mt-1 space-y-2 border-l-2 border-sky-200 pl-3">
                <div>
                  <div
                    class="mb-0.5 text-[10px] font-semibold uppercase tracking-wide text-ink-faint"
                  >
                    {$t("Arguments")}
                  </div>
          ...
          <div data-testid="conversation-tool" class="my-1.5 font-mono text-xs">
            ...
            >
              ...
              <span class="text-ink-muted"
                >{$t("tool result")}{#if part.name}
                  <span class="font-semibold text-ink-strong">{part.name}</span
                  >{/if}</span
              >
            </button>
            {#if !full && peek.lines.length}
              <div
                class="mt-0.5 border-l-2 border-border pl-3 text-[11px] leading-snug text-ink-muted"
              >
                {#each peek.lines as line, li (li)}<div
        ...
        {:else if part.kind === "tool_call"}
          ...
          <div data-testid="conversation-tool" class="my-1.5 font-mono text-xs">
            ...
            {#if full}<div class="mt-1 border-l-2 border-sky-200 pl-3">
                <JsonViewer value={part.args} />
              </div>{/if}
          </div>
        {:else}
          <details data-testid="conversation-tool" class="my-1">
            <summary class="cursor-pointer font-mono text-xs text-ink-muted"
              >● {$t("Other content")}</summary
            >
            <div class="mt-1 border-l-2 border-border pl-3">
              <JsonViewer value={part.value} />
            </div>
          </details>
        {/if}
      {/each}
