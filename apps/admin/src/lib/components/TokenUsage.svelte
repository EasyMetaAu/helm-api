...
<dl data-testid="token-usage" class="grid grid-cols-2 gap-x-6 gap-y-1 text-sm">
  ...
  <dd data-testid="tokens-non-cached" class="text-right font-mono text-ink-strong">
    {fmt(usage.nonCached)}
  </dd>

  <dt class="text-ink-muted">{$t('Cache write tokens')}</dt>
  <dd data-testid="tokens-cache-write" class="text-right font-mono text-ink-strong">
    {fmt(usage.cacheCreation)}
  </dd>

  <dt class="font-medium text-ink-body">{$t('Total tokens')}</dt>
  <dd data-testid="tokens-total" class="text-right font-mono font-semibold text-ink-strong">
    {fmt(usage.total)}
  </dd>
</dl>
