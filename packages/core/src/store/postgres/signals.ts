...
// Postgres adapter for the SignalStore port — the supabase implementation
// (POST-MVP Agentic Signals, docs/02). One row per (task_type, lane);
// `upsertSignals` overwrites via PRIMARY KEY so re-collecting a window never
// double-counts. Stores ONLY aggregate dimensions — no key/payload (principle 7).
// Written off the request path by the background collector.
