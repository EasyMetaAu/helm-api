...
| — | [TTS API](15-tts-api.md) | xAI / SuperGrok OAuth TTS voices lookup and one-shot speech generation contract. |
| — | [Jev Decisions API (中文)](integrations/jev-decisions.zh-CN.md) | Typed external decisions, model permissions, metadata-only audit, usage limitations, and integration examples. |
...
| — | [Grok Video Duration & Extension Plan](grok-video-duration-and-extension-plan.md) | Reviewed plan for one shared 6/10/15/30-second contract across prompt-only, single-image, multi-reference, and account-pinned `/v1/videos/extensions` writes. |
