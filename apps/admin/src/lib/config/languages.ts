...
export const SUPPORTED_LANGUAGES: ReadonlyArray<{ code: LocaleCode; native: string }> = [
...
export function normalizeLocale(input: string | null | undefined): LocaleCode {
  ...
  if (v === 'zh-hant' || v === 'zh-tw' || v === 'zh-hk' || v === 'zh-mo' || v.includes('hant')) {
    return 'zh-hant';
  }
  if (v === 'zh-hans' || v === 'zh-cn' || v === 'zh-sg' || v === 'zh' || v.startsWith('zh-')) {
    return 'zh-hans';
  }
