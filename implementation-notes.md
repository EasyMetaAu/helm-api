...
- **实现**：① `MemoryStore` 端口加 9 个**全 optional** 管理方法（listMemoryScopes / get·list·update·deleteFact / list·get·updateText·deleteReflection），sqlite+pg 双适配器；`insertFactsReconciled` **加可选返回** `MemoryFactReconcileResult{insertedIds,supersededIds}`（additive，旧 caller 忽略；两处 test fake 补返回）。② MCP server 用**直写 JSON-RPC**（不引 @modelcontextprotocol/sdk）：6 工具用 `type:fact|reflection` 判别符，工具定义 transport 无关放 `routes/mcp/tools.ts`，HTTP/dispatch 在 `index.ts`，复用 `authMiddleware`→identity→accountId+默认 project，gating `config.memory.mcp.enabled`（默认 **false** fail-closed，store 不支持则 /mcp 不挂=404）。③ admin 路由 `/admin/api/memory/*` 走既有 basicAuth；SPA 仿 keys 页（Svelte5 runes、Modal、app.css 配方、formatTimestamp、`$t` 英文兜底）。
- **关键决定/坑**：①**管理读不套 `expired_at IS NULL`**——运营者要看得到 superseded/archived/pruned（status 过滤显式：admin 默认 `all`、MCP 默认 `active`）。②`updateFact` 改 factText **重算 content_hash 但不动 subjectKey**（supersede 身份）；与 `UNIQUE(owner_id,content_hash)` 兄弟行碰撞 → typed `MemoryFactContentHashConflictError` → route **409**（绝不冒成 500）。③fact 删除=**软删 `pruned`+盖 expired_at**；tombstone 占 hash → 同文本重加会被 dedup（v1 改写措辞才能重加，工具描述写明）。④reflection 编辑**就地改文本不 bump version**（version 留给 Reflector 机器合并计数），重算 tokenEstimate。⑤`listMemoryScopes` 用 UNION-of-grouped-subqueries（sqlite 无 FULL OUTER JOIN），reflections 读一律 `owner_id IS NOT NULL`（该列可空）。⑥pg Drizzle `.update().returning()` 本仓库只暴露**无参**重载（带列对象报 TS2554）。⑦MCP transport 选直写而非 SDK：SDK 稳定版 `StreamableHTTPServerTransport` 是 Node-req/res 式、`app.request()` 测不到；CRUD 不需 SSE；留迁移注释待 `@modelcontextprotocol/server` 2.0 GA。
- **限制/TODO**：MCP 仅 CRUD 非流式；**"By Key" 实为 account+该 key 默认 project** scope（多 key 共享 account 即共享记忆，UI 已注明）；MCP add 复用 `buildReconciledFactBatch`（含 supersede）cap=1；**未做活体 smoke**（curl /mcp + 浏览器 /memory）、未部署；SPA 未加 5 语言 locale（`$t` 英文兜底）。
- **Codex review 修的 3 个 lifecycle bug**：①`deleteReflection` 原只 archive 单个 id——但 upsertReflection **追加版本不归档旧版**，同 scope 可多个 active 版本、getReflection 取最高版，故删一个 id 后会回落到下一版**注入照旧**。改：按 id 查出 scope → archive 该 scope **全部 active 版本**（复用 reflectionScopeWhere），双适配器同步。②`updateFact` 改 status **须同步 expired_at**：改回 `active` 要 **清 expired_at**（否则 supersede/prune 的 tombstone 还在，active 读 `status='active' AND expired_at IS NULL` 仍看不到=复活无效），改 `pruned` 要**盖 expired_at**（与 deleteFact 一致）；`archived` 不动 expiry（status 已隐身）。③`latestPerScope` 的 tuple key 误用了 **NUL 字节分隔符**（git 把 memory-store.ts 当二进制、行 diff 消失）→ 改 `JSON.stringify([projectId,resourceId,threadId])`（文本安全 + 防 scope id 含空格的分隔符碰撞）。
- **验证**：TDD，新增 sqlite-admin 19 / pg-admin 6 / MCP 12 / admin-route 8 / SPA 4 测（含 3 个 review 回归测）；全量 **277 文件 4047 测**绿（1 个 `admin-oauth.test.ts` 随机密文偶含明文 `RT` 子串的既有 flake，隔离重跑 42/42 绿，与本改无关）、typecheck（4 项目）/ lint(0 err 0 warn) / build / svelte-check(0) 清。分支 `worktree-memory-admin-mcp`，未提交/未部署。
