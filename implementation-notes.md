...
## 2026-06-13 · Phase 4 — memory 前缀注入（docs/08；完整 tool/多模态 + 直通）（原则 1/3/7/8）

- **模型变更：full-replace → 前缀（prefix）。** 旧 inject 把整段会话重建成纯文本 `AssembledMessage[]`（`content:string`）做**全替换**——会**销毁**实时 `tool_calls`/图片/`tool_result`，所以 D7 闸（`isPlainTextTurn`）对一切 tool/多模态/`developer` 回合**跳过注入**，导致 codex/claude-code/openclaw 这些**全都用工具+图片**的真实 agentic 客户端的 inject **形同失效**。新模型：assembler 不再重建会话，只产出**一个系统级文本块**（`buildMemoryBlock`：`# Persistent memory` + Project/Resource knowledge + `## Earlier context (summarized)`，仅有内容的 section 才出，确定性、缓存友好），由 pipeline **前置**到客户端**逐字保留**的实时会话之上。memory 纯**加性**、永不替换实时回合 → 不可能损坏结构 → **D7 闸彻底删除**（tool/多模态/`developer` 回合与纯文本回合一视同仁注入）。
- **窗口感知去重（window-aware dedup）。** 当前请求的 `messages` 就是客户端实时窗口：project/resource reflection **永远注入**（跨线程、客户端从不回发）；thread observation **仅当其覆盖的某个回合已不在实时窗口里**才注入（客户端已压掉它，helm 召回摘要）——覆盖回合**全部仍在窗口**则**跳过**（客户端逐字回发，再注入摘要会重复）。窗口指纹 `sha256Hex(serializeContent(content))` 与 storage 写 `memory_messages.content_hash` **字节恒等**（`serializeContent` EXPORT 自 observe.ts，单一真相源），故实时回合能匹配其持久化 hash。覆盖范围**无法解析**（range 越界 loaded rows）的 observation **保留**（绝不在缺审计行时静默丢召回）；**无窗口**（空集）⇒ 不去重，每条 active observation 都纳入。token budget 仍封顶整块。
- **系统级落点 + 原生直通 splice。** translate 路（core `injectIntoIR`）：会话首条是 system 则把块前置进其 content（空行分隔）、否则**插**一条新 leading system；其余每条 IR 消息按引用、原序、逐字保留，输入数组永不 mutate。原生直通路（gateway `native-memory-inject.ts`）：同一块 splice 进协议原生 system 字段——Anthropic 顶层 `system`（string→前置+分隔；array→插一个 leading `{type:"text"}` 块）、Responses `instructions`（string→前置+分隔）；`messages`/`input` 逐字转发，仅 system carrier 加前缀。**guard 放宽**：`canUseNativePassthrough` 删掉 `memory_mode==="inject"` 禁用项——inject 现在加性、native_request 自洽（memory 在 system 级、实时回合不动），**直通可与 memory 同时触发**（旧 full-replace 重写消息数组才必须在 inject 时禁直通）。
- **observe 路完全未动。** write/learning 路（`observeInbound`/`observeOutbound` + observer/reflector/decay 后台作业）本就以**文本投影**存所有回合类型、无闸，本阶段零改；Observer 自适应压缩策略（size/idle/pressure 触发、catalog 经济学）不变——它只管后台「原始历史→observation」，inject 只**读** observation、不再压缩实时请求。
- **取舍。** ① **放弃实时会话压缩**：helm 不再就地收缩客户端发来的消息数组——客户端自管上下文窗口，helm 只贡献**长期召回**（跨线程 reflection + 客户端已压掉回合的摘要）作加性系统前缀。这正是 inject 对 tool/多模态/直通回合安全（不可能丢结构）的代价。② **prompt-cache 警示**：把 memory 前置到 system carrier 会**位移上游 prompt-cache 前缀**——memory 模式会话可能**上游 prompt 缓存命中下降**，reflection 版本号 bump 也会变前缀（稳定 reflection 则跨回合稳定）。块刻意做成确定性、慢变以最小化 churn，但开启 memory 的会话接受这是召回的代价；`x-memory-mode: off` 会话与无 memory 字节恒等、保留完整缓存。已写入 docs/08（新增「Inject is additive (prefix model)」「Tradeoff」「Native passthrough」三节，删旧 full-replace/D7/recent_raw 契约）。
- **验证 / 改写的旧测试。** 既有断言 full-replace/D7/recent_raw/compaction 的 inject/assembler 测试**全部按前缀模型重写**（非保留）：`inject.test.ts`（19）、`inject-bridge.test.ts`（13）、`inject-forgetting.test.ts`（9）、`messages.inject.test.ts`（14）、`chat.inject.test.ts`（11）、`messages-pipeline.test.ts`（36）、`protocol.test.ts`（12，删 `memory_inject_may_rewrite_request` 期望）；新增 `native-memory-inject.test.ts`（9）。observer/reflector/observe 测试**未动仍绿**。`pnpm typecheck` + `pnpm lint` 干净；`pnpm test` 全量 **3263/3264 绿**——唯一红是 `apps/gateway/src/oauth/admin-oauth.test.ts`「密文不含 'RT'」**预存在 flake**（断言对随机密文 base64 子串，与 memory 无关；隔离重跑 6/6 全绿），**非本分支回归、不在文件范围**。改动文件严格落在范围内（memory/inject.ts、inject-bridge.ts、provider/protocol.ts、messages-pipeline.ts + chat.ts、新 native-memory-inject.ts + 对应测试 + docs + notes）。
- **TODO**：live e2e（真实 codex/claude-code 客户端指向本网关 + memory inject ON + 直通 ON）由用户手验——确认 tool/多模态回合 `memory_hydrated:true` 且 `passthrough_used:true` 同时成立、实时 `tool_calls`/图片字节存活、窗口去重不重复召回客户端仍发的回合。
