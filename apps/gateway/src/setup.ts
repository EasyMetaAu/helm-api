...
const TestProviderSchema = z
  .object({
    ...
    apiKey: z.string().max(8192).trim().default(""),
  })
  .strict();
const CompleteSetupSchema = z
  .object({
    ...
    providerKeys: z.record(z.string(), z.string().max(8192).trim()),
  })
  .strict();
...
export interface SetupServerOptions {
  ...
  testProvider: (providerId: string, apiKey: string) => Promise<{ warning?: string } | undefined>;
...
export async function createSetupServer(opts: SetupServerOptions): Promise<SetupServerResult> {
  ...
  app.post("/setup/api/test-provider", async (c) => {
    ...
    try {
      const result = await opts.testProvider(provider.id, apiKey);
      tested.add(testMarker(provider.id, apiKey));
      const warning = result?.warning ? redact(result.warning, [apiKey]) : undefined;
