<script lang="ts">
  ...
  import { extractConversation, type ConversationTurn } from '$lib/conversation';
  ...
  const firstAssistantIdx = $derived(allTurns.findIndex((tn) => tn.role === 'assistant'));

  const rolesPresent = $derived.by(() => {
    const seen = new Set<ConversationTurn['role']>();
    for (const tn of allTurns) seen.add(tn.role);
    return (['assistant', 'user', 'tool', 'system'] as const).filter((r) => seen.has(r));
  });
...
<div data-testid={testid} bind:this={container} class="flex flex-col gap-2">
  ...
  {:else}
    ...
    <div class="flex flex-wrap items-center gap-x-3 gap-y-2 border-b border-border pb-2 text-xs">
      <!-- role filter chips -->
        ...
        {#each rolesPresent as role (role)}
          {#if role !== 'system'}
            <button
              ...
              onclick={() => (roleFilter = roleFilter === role ? null : role)}>{roleChipLabel(role)}</button
            >
          {/if}
        {/each}
      </div>
      ...
      {#if firstAssistantIdx > 3}
        <span class="text-ink-faint">·</span>
        <button type="button" data-testid="conversation-jump-reply" class="text-ink-muted hover:text-link" onclick={jumpToFirstReply}>
          ↓ {$t('First reply')}
        </button>
      {/if}
      ...
      <span class="ml-auto text-ink-faint">{$t('{count} turns', { count: allTurns.length })}</span>
    </div>
