...
- **实现**：新增 provider 配置 `transport_profile: default|tls_chrome`（默认 `default`，Zod fail-closed）与 env spike 开关 `HELM_EXPERIMENTAL_TLS_TRANSPORT=anthropic`。只允许 Anthropic preset OAuth provider 使用 `tls_chrome`；静态 API key、generic OAuth、其他 provider 显式配置会 fail-closed。执行请求通过 `makeTlsImpersonationFetch()` lazy-load optional `wreq-js`，返回标准 `fetch` seam；per-account proxy 仍从 Helm `ProxyConfig` 转成 wreq proxy URL，凭证只进 transport option，不进日志。OAuth token refresh/model discovery 仍走原 `makeProxyFetch`，本 spike 只改最终 provider execution。
- **取舍/坑/TODO**：选择 `wreq-js` 作为第一版 spike，因为它是 fetch-compatible，避免先移植 OmniRoute `tls-client-node` 的 temp-file SSE tailing。风险仍在：native package/Docker platform、streaming body 行为、wreq proxy 支持矩阵、TLS spoof 是否真能改善 Anthropic API 401/403。默认关闭；若 optional 包缺失，仅在显式选择该 transport 的请求上抛 `TlsTransportUnavailableError`。
- **验证**：新增 core adapter 单测（proxy URL、lazy session、unavailable error）和 gateway wiring 单测（Anthropic preset OAuth opt-in 走 fake wreq session、env 只激活 Anthropic preset、unsupported provider fail-closed）。Focused vitest/typecheck 已覆盖；真正价值必须后续用同一线上账号/同一 request 做 toggle A/B live smoke。
