...
- **互斥留存模式**：运行时新增 `capture_sessions`，与既有 `capture_payloads` 组成「仅元数据 / 每请求完整载荷 / 按会话增量转录」三种互斥模式；两项同时为 `true` 时配置校验 fail-closed，旧实例的 `capture_payloads` 语义不变，设置损坏时的隐私安全回退会同时关闭两种正文捕获。
