...
- **设计**：① `LaneSchema` 加可选 `reasoning_effort`（严格枚举 `ReasoningEffortSchema`，fail-closed，镜像 core `IR_REASONING_EFFORTS`；`z.strictObject` 故旧镜像拒未知字段→**配置不可先于新镜像上线，否则 fail-closed boot**）。② 新内部标志 `InternalRequest.reasoning_effort_forced`，只为通知直通路径改写逐字 body（缺省=直通逐字不动）。③ `route-request` 在 `execPlan.selected_lane` 解析后读 `deps.lanes[lane].reasoning_effort`，命中则 `req.reasoning_effort=值 + reasoning_effort_forced=true`。
- **出站映射**：**翻译路**——chat/responses/gemini 复用既有 `reasoning_effort` 出站映射（stripInternal 已 forward）；Anthropic 新增 `openaiToAnthropicRequest` 里 `reasoning_effort→thinking`。**直通路**——`execute.prepareNativeRequestForUpstream` 加第 6 参 `forcedReasoningEffort`，调新纯函数 `applyForcedReasoningToNativeBody`（`packages/core/src/protocol/reasoning-effort.ts`，与既有 IR-bridge `reasoning.ts` 区分命名）按协议改写 `generationConfig.thinkingConfig` / `reasoning.effort` / `thinking`，记 `body_shims_applied:[reasoning_effort_forced]`。openai_chat 是 lingua franca 不直通→no-op。
- **取舍/坑**：① **Anthropic 扩展思考约束**：注入 thinking 时必须 `max_tokens>budget`、`temperature=1`、剥 `top_p/top_k`（否则 400）——`applyForcedAnthropicThinking` 统一处理两路。预算表 minimal..max=1024..32000（≥1024 floor）。② Anthropic `reasoning_effort→thinking` 现对**所有**互译到 Anthropic 的请求生效（不止 forced），修了 helm 跨协议旧缺口，与 gemini/responses 一致；但带来"openai_chat 客户端 reasoning_effort 到 claude lane 现产 thinking+temperature=1"的行为变化（仅互译路、窄集）。③ `none`：gemini budget 0 关思考、responses 删 reasoning、anthropic 删 thinking。④ **未做**：per-model 默认 / "客户端优先"模式（additive 后续）。
- **验证**：TDD 红→绿。新增 lanes-schema（枚举接受/拒未知）、`reasoning-effort.test.ts`（10）、route-request（forced/非 forced）、anthropic（thinking 派生 + 不覆盖客户端 thinking）、execute（gemini 直通改写仅 forced 时、非 forced 逐字不动）。`pnpm typecheck`/`lint` 绿，**全量 3917 绿**（首跑 272 红是 worktree 未编译 better-sqlite3 原生模块，`pnpm rebuild -r better-sqlite3` 后全绿；非本特性）。分支 `feat/lane-forced-reasoning-effort`（stacked 于 gemini #295）。`config/lanes.yaml` 由用户开了 economy/balanced/coding=high、tool_use=high、premium/coding=xhigh 强制（部署配置，随镜像一起发）。
...
- **复用既有设施（约 80%）**：Helm 已有 Codex OAuth pool、preset `TokenManager`（新鲜 bearer + 代理隧道刷新）、per-account proxy、`codexAccountIdFromToken`，以及与 sub2api 相同的 `/wham/usage` PULL（`apps/gateway/src/oauth/admin-oauth.ts` `fetchCodexQuota`，常量 `CODEX_USAGE_URL`/`CODEX_USAGE_HEADERS` `originator: codex_cli_rs`）。缺的只有：① 暴露 `available_count`；② consume POST + 路由 + UI 按钮。
- **决定**：
  - **额度计数实时、不落库**（消费瞬间即变，落库会「撒谎」）。扩展 `CodexOAuthUsageSchema` 解析 `rate_limit_reset_credits.available_count`（`.loose().nullish()`），新增 `parseCodexResetCredits`/`parseCodexResetResult`（core，fail-open）。`fetchCodexQuota` 返回值由 `OAuthQuotaWindow[]|null` 改为 `{ windows, resetCredits } | null`（唯一调用方是 `/quota` 路由），计数与 windows 同走一次 PULL 并同存 5 分钟缓存。`/quota` 路由把 codex 计数**只挂到响应对象**（不进 quota store，无迁移、不动 `OAuthQuotaSnapshotSchema`）。
  - **consume 走 fail-CLOSED**（与 PULL 的 fail-open 相反）：新增 seam `consumeCodexResetCredit({account})` POST 到 `https://chatgpt.com/backend-api/wham/rate-limit-reset-credits/consume`，body `{ redeem_request_id: randomUUID() }`（幂等键），复用同一 token manager + proxy + `chatgpt-account-id` 头；非 2xx **抛错**（永不回显 body，原则 7），成功后 `quotaCache.delete` 以便下次 PULL 反映重置后的窗口与计数。新增路由 `POST /admin/api/oauth/:provider/reset-credit`：未配置 503、非 codex 400、成功 200、上游失败 502。
  - **UI**：providers 页 Actions 列加 Codex-only「Reset limit」按钮（`codexCredits<=0` 或拉取失败时禁用），Quota 列显示「{n} reset credits」。复用 `savePriority` 的 per-key 进行中状态模式；i18n 走 `pnpm i18n:extract`+`i18n:update`，en + 手译 zh-hans，其余语言回退英文。
- **取舍/坑/TODO**：① 计数挂在已有快照上——若某 codex 账号「有额度但窗口为空」（无快照行），按钮不会出现；但「额度有用」的真实场景必然是已触限（窗口存在），可忽略。② 仅 `openai-codex`，路由显式守卫，未做通用化。③ 未在 admin UI 暴露任何开关，零配置。④ 真机 consume 需有额度的真实账号；否则只能验 502/空额度路径。
- **验证**：TDD 红→绿。shared schema、core 解析器（计数 present/absent/负数/小数、consume 信封）、seam（mock fetch 断言 POST URL/headers/redeem_request_id/缓存失效/fail-closed 抛错）、路由（503/400/200/502 + `/quota` 挂 resetCredits）全覆盖；admin providers 页加 3 个组件测试实点 Reset 按钮（守住 `consumeCodexResetCredit` 导出，补上 Codex review 指出的 mock 缺口）。`pnpm typecheck`、`pnpm lint`、`pnpm build` 绿；全量 `pnpm test`（264 files / 3830 tests）绿（修了一处旧 `admin.test.ts` 对 `fetchCodexQuota` 旧返回形态的 mock；`admin-static.test.ts` 需先 `pnpm build` 产出 admin 静态资源）。分支 `worktree-feat+codex-reset-credit`，本地实现，未部署。
...
## 2026-06-17 · OAuth 账号「使用量限制」自动停靠 + 一键重置（issue: auto-park；原则 2/3/7）

- **背景**：移植 sub2api 的「codex 使用量重置」按钮（`/admin/accounts/:id/clear-rate-limit`，清上游账号 `rate_limit_reset_at` 等让其回池）。但 Helm 架构不同：`oauth_quota` 窗口快照此前**纯展示**，调度（`pool.ts`）只认 `schedulable` 手动停靠，账号撞 ChatGPT/Codex 限额后仍被持续投递。所以「重置」要先把它要重置的**门控**建出来。用户拍板：full parity（真实路由门控）+ 覆盖全部 OAuth provider。
- **实现**（全链路，TDD 红→绿）：唯一新状态是每账号冷却时间戳 `usageLimitedUntilMs`，落在**既有 `oauth_quota` 表新列**（不新建 store、不进 `AccountSettings` blob——让 auto-park 与 operator 手动 park 彻底解耦，是 reset 按钮成立的前提）。
  - **core `pool.ts`**：`OAuthPoolMember` 加 `usageLimitedUntilMs`，`select()` 在 `schedulable` 门后加 `now<until` 跳过（含 sticky 分支，避免把会话钉在刚限流的账号）；返回 `OAuthPoolClient.setUsageLimit(account,until)` O(1) 原地翻转（**单个 429 不重建整池**，重建会重刷每个 token）；**自动恢复 = 每次 select 重判时间戳，无定时器/扫描**；全员被限仍走既有 fail-closed throw → 链推进。
  - **store**：`OAuthQuotaStore.setUsageLimit(provider,account,untilMs|null)`（无快照时合成空行——429 可在任何 PULL 前先停靠）；`upsert` 参数改 `Omit<…,'usageLimitedUntilMs'>`，**编译级**保证窗口刷新永不覆盖活动冷却。sqlite **v26** / pg **v25** 加 nullable 列（rebase 到 main 后 v25/v24 被 true-TPS `generation_ms` 占用，本特性顺延一位）。
  - **检测（只靠实时流量证据）**：① `execute.ts` 真 429（非 `:free`）落在 OAuth 订阅 alias → `onOAuthSubscription429(alias)`，server.ts 从 serving-account ALS 取实际账号，停靠 `DEFAULT_429_COOLDOWN_MS`(60s 短探活)；② `captureCodexQuota`(Codex 每条回包的 `x-codex` header push) 用纯函数 `windowsToUsageLimit`（`usedPercent>=100` 且 reset 在未来）停靠到**精确 reset**。全部 fire-and-forget fail-open。**`/quota` 的 page-open PULL 故意不停靠**（见取舍④）。
  - **跨重启持久化**：`synthesizeOAuthProviders` 加 `usageLimitSeeds`，从 `store.oauthQuota.getAll()` 播种每个 member（重启/重建不误解停；过期种子无害，select 视 `now>=until` 为可用，自清）。
  - **重置**：`POST /admin/api/oauth/:provider/reset?account` → 守卫 `seam() && applyUsageLimit`（OAuth 未启用时 503，绝不写入合成 quota 行——与其它 `/oauth/*` 路由一致）→ `deps.applyUsageLimit(...,null)`（活 member + 持久双清，**无 rebuild、不碰 `schedulable`**）；providers 页加「Rate limited — auto-recovers in …」pill + 仅限流时显示的 **Reset usage** 按钮 + 5 语言 i18n。
- **取舍/坑/TODO**：① `UpstreamError` 不带 header，故 **429 路径只用 60s 短冷却重探**；真撞 5h/周限的精确长冷却走 Codex `x-codex` header-push 路径。若上游 429 不带 header，则每 60s 放一个探活请求（可接受的降级）。**TODO**：可扩展 `UpstreamError` 携带 `retry-after` 让 429 路径也精确长冷却。② **operator park(`schedulable=false`) 与 auto-park 两条独立门**，reset 只清 auto-park；auto 恢复也不会解 operator park。③ 新迁移触 `oauth_quota`（v12 创建），**4 个 sqlite 部分种子 fixture**（schema / memory-schema / memory-dedup-migrate / oauth-usage-migrate）+ pg `migrate.test` 需补 v26/v25 pre-mark（沿用既有 out-of-scope 模式）——凡 pre-mark 过 v12 而不真建 `oauth_quota` 的 fixture 都要补。④ **停靠只靠实时流量证据，`/quota` page-open PULL 不停靠**（Codex review P2）：否则点 Reset → `invalidateAll()` 重新 PULL → 同一仍 saturated 的窗口被立即重新 derive 停靠，账号一次请求都发不出，按钮形同虚设。代价：Anthropic 无 header-push，纯靠 429 的 60s 重探（PULL 本就只在管理页打开时触发、生产几乎不跑，损失极小）；Copilot 无 quota 端点，本来就仅靠 429。⑤ reset 对“窗口确实仍 saturated”的账号语义是“探活一次”：发请求→Codex header 证实仍超→精确重停 / Anthropic 429→重停，属预期。**TODO**：若要在 reset 后抑制重停一段时间（保留 PULL 停靠价值），需加每账号 `resetAck` 时间戳——本次按 Occam 直接去掉 PULL 停靠。
- **验证**：新增 `usage-limit` / `pool` / sqlite+pg `oauth-quota` store / `oauth-quota-migrate` / `execute` 429 / admin `oauth` reset route（含 OAuth 禁用时 503 不变更）单测；admin providers 页 mock 补 `resetUsageLimit`。分支 worktree `feat/oauth-usage-auto-park`。`pnpm typecheck`(shared/core/gateway)、`pnpm lint`、`pnpm build`(含 admin svelte-check) 绿；全量 `pnpm test` 绿。仅本地，未部署。Codex review 三项（P2 reset 被 PULL 撤销 / P2 providers mock 缺 export / P3 禁用守卫）已修。
