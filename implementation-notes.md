...
## 2026-06-19 · 自动数据清理 / 保留 / 归档（admin「Data cleanup」；原则 1/2/3）

- **背景**：网关唯一的自动清理是 `request_payloads` 的 `prunePayloads`，有三个缺陷（代码 + 线上 316MB DB 实证）：①payload prune 是**机会式**且只在 `if(captureEnabled)` 分支里入队（`chat.ts:514`/`payload-capture.ts:145,409`）——关闭 capture 或零流量则永不清理；②`telemetry`（决策记录表）**完全无 prune**，无限增长；③记忆保留 `pruneRetainedMemory` 被 `forgetting.enabled`（默认 false）门控，`memory_messages` 无任何按龄清理。用户要：修这三个 + 所有可调参数进设置 + 定时器 + 全表分类 + UI 开关列表 + 立即清理按钮 + 归档压缩备份。
- **实现（10 slice，TDD，git worktree `worktree-data-cleanup-archival`）**：① **Store 端口**新增可选方法（遵循 `pruneExpiredMemory?` 加法约定，旧 double 不破）：`TelemetryStore.pruneTelemetry/count+selectTelemetryOlderThan/count+selectPayloadsOlderThan`（keyset 翻页，sqlite+pg）、`MemoryStore.count/select/pruneMessagesOlderThan`（FK 安全——messages 是 threads 的子表，按龄删不孤儿）+`pruneFinishedJobsOlderThan`、`OAuthUsageStore.count/pruneUsageOlderThan`、`StoreSet.vacuum()`（sqlite `VACUUM`/pg no-op）。`prunePayloads` 签名**保持** `Promise<void>` 不动（避免破坏 ~7 处既有 mock）。② **core 新模块 `cleanup/`**：纯函数 `cleanup-plan`（settings+now→action[]，master off→[]）、`cleanup-runner`（**核心不变式：归档 fsync+sha 校验成功后才删；归档失败/无 sink→跳过删除，over-retain**；逐表 fail-open）、`scheduler`（独立 setInterval+unref+fail-open+overlap 守卫，**不**并进 60s memory tick）、`archive/`（`ArchiveSink` 端口 + `LocalVolumeSink`：free-space 预检→temp 写→fsync→原子 rename，sha256 over 压缩字节，gzip-JSONL+manifest.json）、`run-pass`（plan→run→把 report 存 config_kv 键 `cleanup_last_run`，scheduler 与「Clean Now」共用）。③ **设置**：`runtime-settings.schema.ts` 加 `cleanup_enabled`(默认true)/`cleanup_interval_hours`(24)/`cleanup_archive_enabled`(true) + 每类 `*_cleanup_enabled`+`*_retention_days`（telemetry 90 / payloads 复用 `payload_retention_days` 30 / oauth_usage 180 / memory_jobs 30 全默认 ON；memory_messages 180 / memory_derived 365 默认 **OFF** 高训练价值+FK 风险须 opt-in）。④ **gateway** 装配（server.ts 外层定义 `runCleanupPassNow`，admin 路由 + scheduler 共用；新 `routes/admin/cleanup.ts` 4 端点 + `cleanup-fs.ts` 列档/路径穿越守卫）。⑤ **admin UI**：settings/+page.svelte 加「Data cleanup」卡（master+interval+archive 开关、6 类 toggle+天数、Clean now / Compact database 按钮、上次运行报告、归档下载链接），`lib/api/cleanup.ts` 客户端。
- **取舍/偏离 plan/坑/TODO**：① 用户四问全选推荐项：ON+archive-first / 本地卷+下载 / 仅归档过期行(不做全库快照) / 手动 VACUUM 按钮。② **范围收敛（Occam）**：v1 只清真正无限增长的表（telemetry、payloads、memory_messages、oauth_usage、memory_jobs + 复用 pruneExpiredMemory 的 memory_derived）；**Category B 有界表（rate_limit/budget/signals/oauth_quota）推迟**——它们按 key/账号数有界、清理对磁盘几无影响，加 8 方法+测试+UI 行不值当，记为后续项。③ **无迁移/无新表**：上次运行报告存 config_kv 而非 plan 里设想的 `cleanup_runs` 账本表（更简单、足够「上次何时清了什么」）；既有 created_at 索引已够，未加新索引。④ `cleanup_interval_hours` 引导期 boot 时读一次，运行时改需重启（master switch `cleanup_enabled` 在 tick 内活读，可热生效）；记为已知限制。⑤ 归档目录 env `HELM_ARCHIVE_DIR`（默认 `<dataDir>/archive`，operator-owned，**非**运行时设置以防 web 端路径注入）。⑥ scheduler 由 `HELM_CLEANUP_DISABLED=1` 整体关（测试默认仍跑，故全量测试用该 env 跑）。⑦ 归档会把 payload 明文落到归档文件（继承 capture 的明文权衡，归档目录与 DB 同信任域）。
...
- **实现（前端）**：列表加 "Usage (24h)" 列（fail-soft 到 []，零流量显 —）+ key 名链入详情；详情页 loader（key=硬 404，stats/requests 各自 fail-soft）；新纯函数 `key-detail-filters.ts`（预设 RANGE + 自定义日范围，**自定义优先**；rollover-safe 日期校验拒 `2026-06-31`；end-day **含**=midnight+1d；按跨度选 hour/day bucket）；详情页复用 dashboard 卡片/图表 + 紧凑请求表+分页。
- **取舍/坑/偏离**：①**Hono 路由顺序**——`/keys/usage` 必须注册在 `/keys/:id` 之前，否则 "usage" 被当 `:id` 捕获（已加路由顺序守卫测试）；②usage 端点窗口解析**复用 `StatsQuerySchema`**（只取 start/end，忽略 bucket/tz/key_id），省一个新 schema；③**cost 全链路保持 nullable**（"未计量"≠ 实测 0）；④`$state(data…)` 触发 svelte state_referenced_locally 警告 → 用 `untrack`（镜像 keys 列表页）；⑤**LayerChart 在 import 时即调 `window.matchMedia`**（jsdom 无），故详情页组件测试必须 `vi.mock('layerchart')`——dashboard 先例是只测 loader 不渲染组件；⑥admin 测试环境 `base=''`，href 断言用 `/keys/:id`；⑦**i18n**：12 个新 key 加到 en+zh-hans+zh-hant（意译），**ja/ko 暂回退英文**（机制支持，列为后续翻译 TODO）。
