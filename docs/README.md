...
| — | [Salient-Fact Memory (Proposal)](salient-fact-memory-spec.md) | **Design proposal (not yet implemented).** Why a short "remember X" turn forms no cross-session memory, and a fix: eager fact extraction decoupled from compaction + deterministic scope-filtered fact injection (the formation+injection layer below the deferred P8 retrieval; no embeddings). |
