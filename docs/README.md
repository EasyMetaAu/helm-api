...
| 13 | [Memory Admin & MCP](13-memory-admin-and-mcp.md) | The admin `/memory` page (browse facts & reflections by key or scope, edit/remove) and the `/mcp` server that exposes memory CRUD + filtering as MCP tools. Adds the optional management methods to the MemoryStore. |
