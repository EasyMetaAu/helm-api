...
- **验证**：新增 golden fixture `packages/core/src/provider/fixtures/claude-cli-strict-tool.ts` 与两条 TDD 回归（非流式 wire shape + tool name restore、流式 restore）。`pnpm exec vitest run packages/core/src/provider/anthropic.test.ts -t "strict Claude CLI tool pipeline|strict-cloaked"` 绿；`pnpm exec vitest run packages/core/src/provider/anthropic.test.ts` 93 绿；`pnpm --filter @helm/core typecheck` 绿；`pnpm exec biome check packages/core/src/provider/anthropic.ts packages/core/src/provider/anthropic.test.ts packages/core/src/provider/fixtures/claude-cli-strict-tool.ts` 绿。
