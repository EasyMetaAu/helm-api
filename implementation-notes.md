...
## 2026-06-13 · 原生协议透传 Phase 3 — openai_responses（Codex）同协议直通（docs/05；原则 1/5/8）

- **范围 / 同协议事实**：把原生透传从 anthropic 扩到 **openai_responses**——当 `/v1/responses` 请求路由到 codex pool（`target_provider_protocol=openai_responses`）时，把客户端**逐字 native Responses body**（仅 `model` patch 成解析后上游 id）转发上游、把上游 native Responses SSE **字节直传**回客户端，跳过 `responses→IR(openai-chat)→responses` 的浪费往返（会 mangle `reasoning.encrypted_content`/tools）。真实 Codex CLI 抓包证实：客户端发**完整** native body（model/input/instructions/`include:["reasoning.encrypted_content"]`/reasoning.effort/`store:false`/`stream:true`/tools/tool_choice/parallel_tool_calls/text/prompt_cache_key/client_metadata），路由到 `openai-codex/gpt-5.5`、`provider_model=gpt-5.5`、`provider=openai-codex`——**真·同协议**，verbatim-forward + model-patch 即足够。**Gemini 出局**（`createProviderClient` 无 gemini 分支，无原生上游 client，推迟）。
- **机制全协议中立，本阶段零改 execute/guard/serialize-client/pool**：guard `canUseNativePassthrough`（#218）本就协议中立——openai_responses 源≠通用语、`request.protocol === targetProviderProtocol` 自然放行；execute 的透传分支早已泛化（`{...native_request, model: providerModel}` + `provider.nativePassthrough/Stream`，per stream/non-stream 特性探测）；serialize-client / oauth pool（Phase 2）已泛化转发。**Codex client 侧**（`provider/openai-responses.ts`）已加 `nativePassthrough`（复用 `requestWithRetry`/`headers`/401-retry/scrub/`errorFromResponse`/`onResponseMeta`，跳过 `openaiToResponsesRequest`+`aggregateResponsesStream`）+ `nativePassthroughStream`（body 含 `stream:true` 逐字发，`readResponsesSSERaw` 字节直传，复用 `readChunkWithIdle` idle 守卫），usage 提取器 `usageFromResponsesResponse`/`usageFromResponsesSSE`（数学镜像 `aggregateResponsesStream`：Responses 把 cache 计**在 `input_tokens` 内**，与 Anthropic cache 分离不同）已在 payload-capture 就位。
- **本阶段（Stage 3）实改两处**：① **messages-pipeline.ts 的透传 tee 改为协议感知**：SSE 帧分割 `splitAnthropicSSEFrames`→`splitSSEFrames`（两协议 wire 成帧恒等，复用）；usage tee `protocol==="openai_responses" ? usageFromResponsesSSE : usageFromAnthropicSSE`；assistant 文本 tee 新增 `accumulateResponsesAssistantText`（从 `response.output_text.delta` 的 string delta 累积，区别于 Anthropic 嵌套 `text_delta`）；usage-bearing-frame 过滤泛化（Responses 认 `response.completed`/`response.incomplete`，Anthropic 认 `message_start`/`message_delta`）。`collect()` 非流式旁路：openai_responses 用 `usageFromResponsesResponse` + 新增 `assistantTurnFromNativeResponses`（读 `output[].content[].output_text`）。**Anthropic 行为逐字节不变**（回归 case 守住）。② **responses.ts route 镜像 messages.ts**：两路（流/非流，Codex 是 stream-only）都把 `ir.metadata.native_request = native` 盖戳（永不日志）；流循环 `result.nativePassthrough===true` 时**绕过** `transformStreamOut` 直写 verbatim `{event,data}`；非流 collect `body = nativePassthrough ? collected : transformResponseOut(collected)`。
- **prelude 去重接缝（Responses 独有，Anthropic 无此问题）**：route 在路由前先发**合成** `response.created`/`response.in_progress` prelude（带 route 分配的 stable id，让流早开）。透传路上游 SSE 自带**真实** prelude 帧——`isResponsesPreludeEvent` 泛化为同时认 `event.type`（translate 路 IR 事件）与 `event.event`（透传路 verbatim 帧），把上游 prelude 丢弃避免重复，其余内容/delta/`response.completed` 逐字节中继。**取舍**：客户端看到的是合成 prelude（我们的 id）+ 上游真实剩余流，而非上游自己的 `response.created`；这与 translate 路完全一致，是既有契约。
- **验证**：TDD 红→绿。messages-pipeline.test.ts 加 openai_responses 透传 streamIR + collect 各 4 例（verbatim 帧、`usageFromResponsesSSE` 回填、`response.output_text.delta` assistant 文本、budget settle、Anthropic 透传保持绿）；responses.test.ts 加 native_request 盖戳（流/非流）、非流 verbatim body 跳 translate-back、流 verbatim 帧跳 transformStreamOut + prelude 去重断言、非透传仍翻译。三目标文件 91 绿，gateway routes 全量 **459 绿**，`pnpm typecheck` + 改动文件 `biome` 干净。
- **已知限制 / TODO**：Gemini 推迟（无原生 client）；非流 Responses 透传 collect 罕被触发（Codex stream-only）但保持正确；live e2e（真实 Codex CLI 指向本网关 + flag ON）尚未在本阶段跑，建议合并前手验 `passthrough_used:true` + `reasoning.encrypted_content` 字节存活。

## 2026-06-13 · 原生协议透传 Phase 2 — Anthropic 流式直通（docs/05；原则 8）

- **核心设计：字节级 SSE 直传，而非「解析→重序列化」。** 流式透传把上游 Anthropic SSE 原样转发，所以 `convertOpenAIStreamToAnthropic` 这台 SSE 重映射状态机（#221/#222 保真 bug 的真正住所）在透传路径上被**彻底消灭，而非替换**：data JSON 载荷逐字节到达客户端，只有标准 SSE 信封（`event:`/`data:`/`\n\n`）由 Hono `writeSSE` 重新成帧（语义恒等，与今天每个响应同款）。**流式透传消灭 SSE 重映射风险（原则 8），而非增加。**
- **原始读取复用既有 idle 守卫，不翻译**：`provider/anthropic.ts` 新增 `nativePassthroughStream`，body 原样发上游、用新增 `readAnthropicSSERaw` 读回——它复用 `translateAnthropicSSE` 的 `readChunkWithIdle` 循环（同 idle 守卫 + `StreamStalledError`→`UpstreamError('timeout')`），但 **yield 原始解码文本，零翻译**。OAuth pool client 转发 `nativePassthroughStream`（select-then-delegate，member 缺方法 fail-closed）。
- **breaker 契约不变 + peekStream 重构为 stream thunk（Stage 3）**：`peekStream(open: () => AsyncIterable<string>, …)` 改吃一个 thunk，**同一 peek/relay/breaker 逻辑同时服务两条路**——translate 路传 `() => chatCompletionStream(stripInternal(req))`，透传路传 `() => nativePassthroughStream(native_request)`；首包前抛错→reject→`recordFailure`+链推进，健康→`recordSuccess`。thunk 在 peekStream 内部调用，打开 iterable 的同步 throw 仍被 caller 的 per-candidate try/catch 接住，与今天一致。
- **guard 现在放行流式**：`canUseNativePassthrough` 删掉 `stream_not_supported_phase1`，对流/非流协议中立——provider 是否能流式透传由 `providerSupportsPassthrough` 表达（execute 对流请求特性探测 `nativePassthroughStream`、非流探测 `nativePassthrough`），stream 不再是 blocker。
- **治理 tee，永不改转发字节（原则 8）**：转发的 SSE 帧之外，**一份拷贝**喂 usage 提取 / observe-outbound assistant 文本累积 / payload 捕获——绝不改动转发字节。usage 经新增 `usageFromAnthropicSSE` 回填：input/cache 取自 `message_start`、output 取自末条 `message_delta`（cache_* 跨帧取 max），归一成 Anthropic 形 `StreamUsage` 后喂 cost/budget/OAuth。`native_request` carrier 现在对**流式也盖戳**（messages.ts 的 `ir.metadata.native_request` 不再仅非流式）；messages.ts 流式循环在 `result.nativePassthrough === true` 时**跳过** `transformStreamOut`、直写 native 帧。
- **回归保持 GREEN**：flag 默认 OFF，透传不碰 translate 路径——#221/#222 保真回归、`convertOpenAIStreamToAnthropic`（protocol-matrix 176 + stream 19 + footguns 13）、chat/responses/gemini 流式全部逐字节绿。Stage 5 全量验证：`pnpm typecheck` + `pnpm lint` 干净，`pnpm test` **244 文件 / 3208 测试全绿**。
- **live e2e（真实 Claude 订阅，本地 docker）抓出 2 个纯 stub 单测漏掉的集成 bug，已修（commit 384c153）**：① `createSerializingClient`（#93 用户消息串行队列装饰器，启用队列时包在每个 OAuth pool 成员外）只转发 `chatCompletion`/`chatCompletionStream`，**吞掉了新方法** → pool 特性探测判定成员不支持 → fail-closed（`oauth pool member does not support native passthrough`）。修复：按**同一 lease** 转发 `nativePassthrough`/`nativePassthroughStream`，**仅当 inner 具备时才暴露**（保持上游特性探测诚实）。② execute 透传逐字转发 native body 含 `model`，但那是**路由别名**（`anthropic/claude-…`）非上游真实 id → 上游 **404** `not_found_error`。修复：把 `model` patch 成解析后的 `providerModel`（镜像 `stripInternal`），其余字段逐字。**教训：OAuth 订阅执行链是 `pool → serializing 装饰器 → native client`，Explore/Plan 都没浮现 serialize-client 这层；纯 stub 单测还把「逐字转发含 model」错误地固化进断言，靠真实上游 e2e 才暴露。已让 execute.test 用别名 `anthropic/claude-x` 复现并锁定 model patch。** 真实验证：非流式 + 流式 `passthrough_used:true`/`status:ok`，原生响应/SSE 字节忠实（`ping` 事件 + `service_tier`/`cache_creation` 等 translate 会丢的字段都在）。**综合 live e2e battery（真实订阅）全绿**：多模态图片（透传 vs 翻译均正确识别）、工具调用（`tool_use` stop=tool_use）、**tool_result 回合（#221 重灾区，正确续答）**、system+多轮、多模态流式、工具调用流式（含 `input_json_delta`）、错误信封（坏图→502 Anthropic 形错误）——#221/#222 保真重灾区经透传全部正确。仍未 live 覆盖（次要）：跨协议 fallback（需 lane 配置，已单测）、真实 Claude Code 客户端、超长流/中途 abort。

## 2026-06-13 · 原生协议透传 Phase 1（docs/02/05；原则 1/7/8）

- **缘起 / 取代 #225**：一个 `/v1/messages` 请求路由到 Anthropic 订阅后端要经 9 种表示、4 次有损翻译（`Anthropic-native → IR → InternalRequest → OpenAI-Chat body → Anthropic wire → [上游] → OpenAI-Chat resp → IRResponse → Anthropic-native`）。这 4 次翻译正是 #221/#222 保真 bug（tool_result 多模态、thinking 历史、cache 字段）的住所；**当入站协议 == 上游原生协议时纯浪费 + 纯风险**。原生透传把客户端原始 body 原样转发上游、把原生响应原样回客户端，同时保留每个治理环节。**本方案取代未合并的 #225**——后者是 openai_chat→openai_chat，零收益（openai_chat 本就是内部通用语，没有翻译可省）；复用 #225 的脚手架（`resolveAttemptTarget`、逐 attempt 协议判定、registry/ROUTABLE_OAUTH 协议元数据、逐 attempt 遥测字段、runtime flag），但重新指向 native passthrough。
- **载体 carrier**：`InternalRequest.native_request?: Record<string,unknown>`（协议由 `InternalRequest.protocol` 标明，一个字段足够），**仅在 anthropic 面经 `ir.metadata.native_request` 交接袋设置**（与 `client_billing_header` 同模式）；chat.ts/openai_chat 永不设（永不透传，无可省）。
- **执行接缝**：新增可选 `ProviderClient.nativePassthrough?(body, opts?)`（其它 client/测试桩零改动，executor 特性探测）；Anthropic client 实现复用既有 HTTP core（`requestWithRetry`→`request`/`headers`/`withTimeout`/401-retry/`scrub`/`errorFromResponse`），**跳过** `openaiToAnthropicRequest`（body 原样发）和 `anthropicToOpenAIResponse`（json 原样返）。`headers(nativeBody)` 已从 body 派生 beta/UA，native body 同形可直接用。OAuth pool client 必须转发 `nativePassthrough`（select-then-delegate，member 缺方法 fail-closed），否则订阅 alias 的探测恒为 undefined、分支永不触发。
- **判定 guard（core `protocol.ts` 纯函数 `canUseNativePassthrough`）**：禁用原因顺序（首个失败胜出，确定性可单测）：`feature_flag_disabled` → `missing_native_request` → `source_protocol_is_lingua_franca`（openai_chat 源显式拒）→ `protocol_mismatch`（`request.protocol !== targetProviderProtocol`，含蓄保证 response==request）→ `stream_not_supported_phase1` → `memory_inject_may_rewrite_request` → `fallback_may_change_provider_protocol`（前瞻后续候选异构链）→ `provider_requires_compatibility_rewrite` → `provider_lacks_passthrough` → ok。execute.ts 是唯一判定 + 分支点（透传分支在现有 per-candidate try/catch 内，breaker/abort/free-429/链推进语义完全不变）。
- **结果旁路（#225 无先例的最 novel 接缝）**：`ExecuteOutcome`/`ExecutionResult`/`PipelineRunResult` 加 `nativePassthrough?: boolean`；`collect()` 命中时原样返回 native body（跳 `openAIBodyToIR`），messages.ts 跳 `transformResponseOut`；cost 用 `usageFromAnthropicResponse`（数学镜像 `anthropicToOpenAIResponse`）归一后喂 `costOf`。两端 payload 都保持 native（原则 7），DecisionRecord 仍 body-free，遥测 +8 个 `passthrough_*` 脱敏字段。
- **Flag**：`same_protocol_serialization_fast_path` → `native_protocol_passthrough`（Zod default false，runtime 可调，admin live-binding `() => settings.native_protocol_passthrough` 同 `capturePayloads` 模式）。**默认 OFF，合并 ≠ 启用**。admin settings client `normalize()` 必须认这个字段（#225 教训：漏了会在每次保存时静默重置）。server.ts 一处 wiring（`route` 闭包内 `createExecute`）同时覆盖 chat.ts 与 messages-pipeline 两面（同一闭包）；`ROUTABLE_OAUTH` 各条带 `targetProviderProtocol`+`requiresRewrite`（anthropic→anthropic_messages、copilot→openai_chat、codex→openai_responses），建 `ROUTABLE_OAUTH_PROTOCOLS` map，synthesis + `buildRegistry` backfill 带协议元数据（防非 anthropic primary 被默认 openai_chat 误标）。
- **Phase 1 范围**：**仅非流式 anthropic→anthropic**。已知限制：(R7) 非 CLI 的 anthropic 客户端缺 `x-anthropic-billing-header` system[0] 块为已知限制（真 Claude Code 的 native body 自带真实块，透传转发比合成更真；Phase 1 决定原样转发，默认 OFF + 仅订阅）；(observe-outbound) 从 native `content[].text` 重建 `{role:"assistant",content}` 是 fail-open 简化。
- **Phase 2（勾勒）**：anthropic 流式透传——加 `nativePassthroughStream?` + 字节 SSE 直传（非 `translateAnthropicSSE`），复用 `peekStream` 首包-成功 breaker 契约 + `readChunkWithIdle` idle 守卫，去掉 `stream_not_supported_phase1`；流式捕获每帧 COPY 喂 `createSseCapture`；usage 从末帧 `message_delta/message_stop` 取。**流式透传消灭 SSE 重映射风险（原则 8），而非增加。Phase 3（勾勒）**：openai_responses（Codex，元数据已在 C4）、gemini 同协议透传——机制已协议无关（各 client 加 `nativePassthrough` + 各面 route 旁路 + 各自 native usage 提取器）。
