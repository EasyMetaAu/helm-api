...
- **#6〔追加，用户拍板〕非自定义模型 key 全走 auto**：model-alias 兼容垫片（route-request Step 0a）原本对任何 key 生效（让固定模型客户端不吃 400），用户要求「只要 `allow_custom_model:false` 就全部走 auto」。给 Step 0a 加 `opts.allowCustomModel===true` 门槛 → 非自定义 key 的 model 字段（含已知厂商 id）一律忽略、走分类（仍不 400）；垫片降为「自定义模型能力」的一部分。同步改 `config/model-aliases.yaml` 注释 + route-request 别名测试（新增「默认 key 忽略别名→分类」回归 + 其余别名用例改 `allowCustomModel:true`）。PR #260（文档）已合并入 main，故 README（中英 model 表 + 「固定模型即插即用」行）与 docs/04（「shim 对任何 key 生效」「priority-0」「lane-name 直通」描述）随本改动一并更正——一句话：标准 key 永远只走 auto，pin lane/厂商/模型都属于自定义模型能力。
