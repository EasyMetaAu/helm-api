...
export function detectProtocol(request: unknown): ClientProtocol {
  ...
  if (asArray(rec.messages))
    return isAnthropicMessages(rec) ? "anthropic" : "openai-chat";
...
function isAnthropicMessages(rec: Record<string, unknown>): boolean {
  ...
  for (const m of boundedArray(rec.messages)) {
    ...
    if (blocks) {
      for (const b of blocks.length > MAX_PARTS
        ? blocks.slice(0, MAX_PARTS)
        : blocks) {
...
function resolveImage(part: Record<string, unknown>): string | null {
  ...
  if (typeof imageUrl === "string")
    return imageDataUrl(imageUrl) ?? httpUrl(imageUrl);
  const iu = asRecord(imageUrl);
  if (iu) return imageDataUrl(iu.url) ?? httpUrl(iu.url);
  const src = asRecord(part.source);
  if (src)
    return imageDataUrl(src.data) ?? imageDataUrl(src.url) ?? httpUrl(src.url);
...
function mapContentPart(part: unknown): TurnPart | null {
  ...
  if (type === "thinking")
    return { kind: "reasoning", text: str(p.thinking) ?? "" };
  if (type === "redacted_thinking")
    return { kind: "reasoning", text: "[redacted thinking]" };
...
function foldResponseItems(
  ...
  const turns: ConversationTurn[] = [];
  const toolNames: ToolIndex = new Map();
  for (const it of items.length > MAX_TURNS
    ? items.slice(0, MAX_TURNS)
    : items) {
    ...
    if (type === "message") {
      turns.push({
        role: normalizeRole(str(item.role)),
        parts: mapContent(item.content),
        raw: it,
        origin,
      });
      ...
      turns.push({
        role: "assistant",
        parts: [{ kind: "tool_call", id, name, args }],
        raw: it,
        origin,
      });
    } else if (
      ...
      turns.push({
        role: "assistant",
        parts: [{ kind: "reasoning", text: summaryText(item.summary) }],
        raw: it,
        origin,
      });
    } else {
...
function foldGemini(rec: Record<string, unknown>): ConversationTurn[] {
  const turns: ConversationTurn[] = [];
  const sysInstr =
    asRecord(rec.systemInstruction) ?? asRecord(rec.system_instruction);
  if (sysInstr) {
    const parts = geminiParts(sysInstr.parts, new Map(), []);
    if (parts.length)
      turns.push({ role: "system", parts, raw: sysInstr, origin: "request" });
  }
...
function systemFromValue(value: unknown): ConversationTurn | null {
  ...
  return parts.length
    ? { role: "system", parts, raw: value, origin: "request" }
    : null;
}
...
function assistantTurnFromAssembled(
  a: AssembledStream,
  raw: unknown,
): ConversationTurn {
  const parts: TurnPart[] = [];
  ...
  for (const call of capParts(a.toolCalls))
    parts.push({
      kind: "tool_call",
      id: call.id,
      name: call.name,
      args: coerceJson(call.arguments),
    });
...
function foldResponseObject(rec: Record<string, unknown>): ConversationTurn[] {
  ...
  if (asArray(rec.content)) {
    const turns = foldAnthropic({ messages: [rec] }).map((t) => ({
      ...t,
      raw: rec,
...
function pairToolExchanges(turns: ConversationTurn[]): ConversationTurn[] {
  ...
  turns.forEach((turn, ti) =>
    turn.parts.forEach((p, pi) => {
      ...
      while (
        idx < list.length &&
        (list[idx].ti < ti || (list[idx].ti === ti && list[idx].pi < pi))
      )
        idx++;
      if (idx < list.length) {
        const loc = list[idx];
...
export function formatToolArgs(
  _name: string,
  args: unknown,
  maxChars = MAX_TOOL_DETAIL_CHARS,
): string {
  try {
    ...
    if (!a) {
      ...
      return parsed == null
        ? ""
        : truncateDetail(JSON.stringify(parsed), maxChars);
    }
    ...
    if (picked.key === "command")
      return truncateDetail(formatBashCommandChain(picked.value), maxChars);
    if (
      (picked.key === "file_path" || picked.key === "filePath") &&
      typeof a.content === "string" &&
      a.content.length > 0
    ) {
      const chars = a.content.length;
      const suffix =
        chars >= 1000 ? ` · ${Math.round(chars / 100) / 10}k` : ` · ${chars}`;
...
function outputToText(output: unknown): string {
  if (typeof output === "string") {
    ...
    return parsed !== output && parsed != null && typeof parsed === "object"
...
export function toolOutputPeek(
  output: unknown,
  maxLines = 6,
): { lines: string[]; moreLines: number } {
  try {
